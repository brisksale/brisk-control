...
const arrayFromIterator = function arrayFromIterator(iter) {
  const list = [];
  let next;
...
const complement = function complement(f) {
  return function () {
...
const functionName = function functionName(f) {
  // String(x => x) evaluates to "x => x", so the pattern may not match.
...
const _isObject = function _isObject(x) {
  return Object.prototype.toString.call(x) === '[object Object]';
};

const _isString = function _isString(x) {
  return Object.prototype.toString.call(x) === '[object String]';
};

const _isTransformer = function _isTransformer(obj) {
  return typeof obj['@@transducer/step'] === 'function';
};

const _map = function _map(fn, functor) {
  let idx = 0;
...
const _quote = function _quote(s) {
  const escaped = s.replace(/\\/g, '\\\\').replace(/[\b]/g, '\\b')    // \b matches word boundary; [\b] matches backspace
    .replace(/\f/g, '\\f').replace(/\n/g, '\\n').replace(/\r/g, '\\r').replace(/\t/g, '\\t').replace(/\v/g, '\\v').replace(/\0/g, '\\0');
  return '"' + escaped.replace(/"/g, '\\"') + '"';
};

const _toISOString = function () {
...
const _dispatchable = function _dispatchable(methodNames, xf, fn) {
  return function () {
...
const _isArrayLike = function isArrayLike(x) {
  if (_isArray(x)) {
    return true;
  }
  if (!x) {
...
const bind = function bind(fn, thisObj) {
  return arity(fn.length, function () {
    return fn.apply(thisObj, arguments);
  });
};

const identical = function identical(a, b) {
  // SameValue algorithm
  if (a === b) {
    // Steps 1-5, 7-10
    // Steps 6.b-6.e: +0 != -0
...
const _equals = function _equals(a, b, stackA, stackB) {
  if (identical(a, b)) {
    return true;
  }
  if (type(a) !== type(b)) {
...
const filter = _dispatchable(['filter'], _xfilter, function (pred, filterable) {
...
const reject = function reject(pred, filterable) {
  return filter(complement(pred), filterable);
};

const _indexOf = function _indexOf(list, a, idx) {
  let inf, item;
...
const _toString = function _toString(x, seen) {
